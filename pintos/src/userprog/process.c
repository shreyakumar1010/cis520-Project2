...
tid_t
process_execute (const char *file_name_plus_arguments) 
{
  char *fn_copy;
  tid_t tid;
...
start_process (void *file_name_)
{
  ...
  if (!success)
  {
     ...
     thread_current() -> parent -> prod_flag = false;
     sema_up(&thread_current()-> parent -> prod_sema);
     thread_exit ();
  }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Set up stack. */
  if (!setup_stack (esp, numOfArguments, NameThenArguments))
    goto done;
